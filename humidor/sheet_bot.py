...
posting_file = "{}/posting.pkl".format(dir_path)
...
creds = ServiceAccountCredentials.from_json_keyfile_name('{}/credentials.json'.format(dir_path), SCOPE)
...
def post_data(posting_data):
    ...
    # Burn posting.pkl file when successful. 
...
def main():
    ...
    if datetime.datetime.now().minute == 59:  # If the hour is about to change, wait a minute. 
        time.sleep(60)
    send_message('Sheet bot is posting data', channel)
    split_data()  # Split raw data into two files: posting data and data.pkl to leave alone 
...
if __name__ == '__main__':
    try:
        ...
        send_message('Sheet bot Error: {}'.format(e), channel)
